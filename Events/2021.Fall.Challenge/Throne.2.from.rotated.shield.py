...
# Shiled will be visible after applying the following hints :
...
def compute(clans: List[dict]) -> str:
    ...
    for c in clans:
        # shield is visible once the text enigma is solved.
        corner = c['corners'][0]
        if not (corner['color'] == 'red' and corner['symbolType'] == 'none'):
            continue

        corner = c['corners'][1]
        if not (corner['color'] == 'white' and corner['symbolType'] == 'disc' and corner['symbolAmount'] == 3 and corner['symbolColor'] == 'blue'):
            continue
